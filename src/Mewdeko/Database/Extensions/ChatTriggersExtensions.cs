...
public static class ChatTriggersExtensions
{
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains the matching ChatTriggers
    ///     instance, or null if not found.
    /// </returns>
    public static async Task<ChatTriggers> GetByGuildIdAndInput(this DbSet<ChatTriggers> crs, ulong? guildId,
        string input)
    {
        return await crs.FirstOrDefaultAsyncLinqToDB(x => x.GuildId == guildId &&
