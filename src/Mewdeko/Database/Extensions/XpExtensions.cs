...
public static class XpExtensions
{
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains the UserXpStats entity for the
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains a List of UserXpStats entities for
    ///     the specified guild, ordered and paged.
    /// </returns>
    public static Task<List<UserXpStats>> GetUsersFor(this DbSet<UserXpStats> set, ulong guildId, int page)
    {
        return set.AsQueryable().AsNoTracking().Where(x => x.GuildId == guildId)
    ...
    /// <param name="guildId">The ID of the guild to filter by.</param>
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains a List of all UserXpStats entities
    ///     for the specified guild, ordered by total XP.
    /// </returns>
    ...
    {
        return set.AsQueryable().AsNoTracking().Count(x =>
                   x.GuildId == guildId
                   && x.Xp + x.AwardedXp
                   > set.AsQueryable().Where(y => y.UserId == userId && y.GuildId == guildId)
