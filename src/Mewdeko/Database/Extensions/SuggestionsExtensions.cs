...
public static class SuggestionsExtensions
{
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains an array of SuggestionsModel
    ...
    {
        return set.AsQueryable().Where(x => x.GuildId == guildId && x.SuggestionId == sugid).ToArrayAsyncEF();
    }
    ...
    {
        return set.AsQueryable().Where(x => x.GuildId == guildId && x.UserId == userId).ToArrayAsyncEF();
    }
}