...
public static class RoleGreetExtensions
{
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result contains an array of RoleGreet entities for
    ///     the specified role.
    /// </returns>
    public static Task<RoleGreet[]> ForRoleId(this DbSet<RoleGreet> set, ulong roleId)
    {
        return set.AsQueryable().Where(x => x.RoleId == roleId).ToArrayAsyncEF();
    }
}