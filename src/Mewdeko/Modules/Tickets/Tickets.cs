...
public class Tickets : MewdekoModuleBase<TicketService>
{
    ...
    public async Task TSetup()
    {
        ...
        if (new ChannelTypeReader<ITextChannel>().ReadAsync(ctx, response, _services).Result.IsSuccess)
        {
            ...
            channel =
                new ChannelTypeReader<ITextChannel>().ReadAsync(ctx, response, _services).Result.BestMatch as
                    ITextChannel;
            if (!await PromptUserConfirmAsync(
                    new EmbedBuilder().WithOkColor().WithDescription(
                        $"Would you like to setup a custom ticket panel embed? If not right now I'll just use the default and you can edit anytime using either {Prefix}tpedit or using the {Prefix}edit command."),
                    ctx.User.Id))
            {
                if (await PromptUserConfirmAsync(
                        new EmbedBuilder().WithOkColor().WithDescription(
                            "Ok! I'll use the default for now! Would you like to setup a custom button emote and text?"),
                        ctx.User.Id))
                {
                }
            }
            ...
            {
                component.WithButton("Support Server", url: "https://discord.gg/wB9FBMreRk",
                    style: ButtonStyle.Link);
                component.WithButton("Embed Builder", url: "https://eb.mewdeko.tech",
                    style: ButtonStyle.Link);
                eb.WithDescription(
                        "Alright! Can you go https://eb.mewdeko.tech and setup the embed? If not we can help you in the support server: https://discord.gg/wB9FBMreRk \nOtherwise just paste the embed code if you know how!")
                    .WithOkColor();
                ...
                if (CREmbed.TryParse(embed, out crembed))
                {
                    ...
                    if (await PromptUserConfirmAsync(
                            new EmbedBuilder().WithDescription("Embed Recorded! Would you like to preview it?")
                                .WithOkColor(), ctx.User.Id))
                    {
                        var msg2 = await ctx.Channel.SendMessageAsync(crembed.PlainText,
