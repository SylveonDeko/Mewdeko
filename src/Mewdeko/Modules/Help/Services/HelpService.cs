...
public class HelpService : ILateExecutor, INService
{
    ...
    public ComponentBuilder GetHelpSelect(IGuild? guild, bool descriptions = true)
    {
        ...
        foreach (var i in modules.Where(x => !x.Attributes.Any(x => x is HelpDissabled)))
        {
    ...
    public EmbedBuilder GetHelpEmbed(bool description, IGuild? guild, IMessageChannel channel, IUser user)
    {
        ...
        var modules = _cmds.Commands.Select(x => x.Module).Where(x => !x.IsSubmodule && !x.Attributes.Any(x => x is HelpDissabled)).Distinct();
        var count = 0;
        if (description)
        {
    ...
    {
        if (com.Attributes.Any(x => x is HelpDissabled))
