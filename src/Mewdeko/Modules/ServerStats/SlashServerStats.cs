using System.IO;
using System.Text;
using Discord.Interactions;
using Fergun.Interactive;
using Fergun.Interactive.Pagination;
using Mewdeko.Common.Attributes.InteractionCommands;
using Mewdeko.Modules.ServerStats.Common;
using Mewdeko.Modules.ServerStats.Services;

namespace Mewdeko.Modules.ServerStats;

/// <summary>
///     Slash commands for activity statistics: messages, voice time, member growth and status over configurable
///     windows, with charts, rankings and exports.
/// </summary>
[Group("serverstats", "Message, voice and member activity statistics")]
public class SlashServerStats(InteractiveService interactivity, ServerStatsSettingsService statsSettings)
    : MewdekoSlashModuleBase<ServerStatsService>
{
    /// <summary>
    ///     Shows the server's activity summary for a window.
    /// </summary>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("server", "Shows the server's activity summary")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task Server([MinValue(0)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var overview = await Service.GetOverviewAsync(ctx.Guild, lookback);
        var topGame = statsSettings.GetCachedSettings(ctx.Guild.Id).TrackActivities
            ? (await Service.GetTopActivitiesAsync(ctx.Guild.Id, lookback, 1)).FirstOrDefault()
            : null;
        await FollowupAsync(embed: ServerStatsEmbeds.Overview(Strings, ctx.Guild, overview, topGame).Build());
    }

    /// <summary>
    ///     Shows a member's activity summary for a window.
    /// </summary>
    /// <param name="user">The member, or the caller when omitted.</param>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("user", "Shows a member's activity summary")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task User(IGuildUser? user = null, [MinValue(0)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        user ??= (IGuildUser)ctx.User;
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var activity = await Service.GetUserActivityAsync(ctx.Guild, user.Id, lookback);
        var games = statsSettings.GetCachedSettings(ctx.Guild.Id).TrackActivities
            ? await Service.GetTopActivitiesAsync(ctx.Guild.Id, lookback, 5, user.Id)
            : null;
        await FollowupAsync(embed: ServerStatsEmbeds.User(Strings, ctx.Guild, user, activity, games).Build());
    }

    /// <summary>
    ///     Ranks the games and apps members spend the most time in.
    /// </summary>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("games", "Ranks the games and apps members spend the most time in")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task Games([MinValue(0)] [MaxValue(90)] int? days = null)
    {
        if (!statsSettings.GetCachedSettings(ctx.Guild.Id).TrackActivities)
        {
            await ReplyErrorAsync(Strings.StatsActivitiesOff(ctx.Guild.Id));
            return;
        }

        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var rows = await Service.GetTopActivitiesAsync(ctx.Guild.Id, lookback, 15);
        await FollowupAsync(embed: ServerStatsEmbeds.Activities(Strings, ctx.Guild, lookback, rows).Build());
    }

    /// <summary>
    ///     Shows who plays a game: members in it right now and the members with the most time in it.
    /// </summary>
    /// <param name="game">The game or app name.</param>
    [SlashCommand("whoplays", "Shows who plays a game or uses an app")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task WhoPlays(string game)
    {
        if (!statsSettings.GetCachedSettings(ctx.Guild.Id).TrackActivities)
        {
            await ReplyErrorAsync(Strings.StatsActivitiesOff(ctx.Guild.Id));
            return;
        }

        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, null);
        var totals = await Service.GetPerUserActivitySecondsAsync(ctx.Guild.Id, game, lookback);
        var now = Service.CountActiveNow(ctx.Guild.Id, game);
        if (totals.Count == 0 && now == 0)
        {
            await ReplyErrorAsync(Strings.StatsNoData(ctx.Guild.Id));
            return;
        }

        var eb = new EmbedBuilder()
            .WithOkColor()
            .WithTitle(Strings.StatsWhoPlaysTitle(ctx.Guild.Id, game))
            .AddField(Strings.StatsPlayingNow(ctx.Guild.Id), now.ToString("N0"), true)
            .AddField(Strings.StatsPlayersWindow(ctx.Guild.Id, ServerStatsEmbeds.WindowName(lookback)),
                totals.Count.ToString("N0"), true)
            .AddField(Strings.StatsTotalTime(ctx.Guild.Id),
                ServerStatsService.FormatDuration(totals.Values.Sum()), true);

        if (totals.Count > 0)
            eb.AddField(Strings.StatsTopPlayers(ctx.Guild.Id), string.Join("\n",
                totals.OrderByDescending(x => x.Value).Take(10)
                    .Select((x, i) => $"`{i + 1}.` <@{x.Key}>: **{ServerStatsService.FormatDuration(x.Value)}**")));

        await FollowupAsync(embed: eb.Build());
    }

    /// <summary>
    ///     Shows a channel's activity summary for a window.
    /// </summary>
    /// <param name="channel">The channel, or the current one when omitted.</param>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("channel", "Shows a channel's activity summary")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task Channel(IGuildChannel? channel = null, [MinValue(0)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        channel ??= (IGuildChannel)ctx.Channel;
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var activity = await Service.GetChannelActivityAsync(ctx.Guild, channel.Id, lookback);
        await FollowupAsync(embed: ServerStatsEmbeds.Channel(Strings, ctx.Guild, channel, activity).Build());
    }

    /// <summary>
    ///     Ranks members by messages or voice time.
    /// </summary>
    /// <param name="kind">Messages or voice.</param>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    /// <param name="role">Only include members with this role.</param>
    [SlashCommand("top", "Ranks members by messages or voice time")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task Top(StatKind kind = StatKind.Messages, [MinValue(0)] [MaxValue(90)] int? days = null,
        IRole? role = null)
    {
        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var entries = await Service.GetTopUsersAsync(ctx.Guild.Id, kind, lookback, 500);

        if (role != null)
        {
            var filtered = new List<TopEntry>();
            foreach (var entry in entries)
            {
                var member = await ctx.Guild.GetUserAsync(entry.Id);
                if (member != null && member.RoleIds.Contains(role.Id))
                    filtered.Add(entry);
            }

            entries = filtered;
        }

        if (entries.Count == 0)
        {
            await ReplyErrorAsync(Strings.StatsNoData(ctx.Guild.Id));
            return;
        }

        var title = Strings.StatsTopTitle(ctx.Guild.Id, ServerStatsEmbeds.KindName(Strings, ctx.Guild.Id, kind),
            ServerStatsEmbeds.WindowName(lookback));
        await SendRankingAsync(entries, title, kind, id => $"<@{id}>");
    }

    /// <summary>
    ///     Ranks channels by messages or voice time.
    /// </summary>
    /// <param name="kind">Messages or voice.</param>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("channels", "Ranks channels by messages or voice time")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    public async Task Channels(StatKind kind = StatKind.Messages, [MinValue(0)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var entries = await Service.GetTopChannelsAsync(ctx.Guild.Id, kind, lookback, 100);
        if (entries.Count == 0)
        {
            await ReplyErrorAsync(Strings.StatsNoData(ctx.Guild.Id));
            return;
        }

        var title = Strings.StatsTopChannelsTitle(ctx.Guild.Id,
            ServerStatsEmbeds.KindName(Strings, ctx.Guild.Id, kind), ServerStatsEmbeds.WindowName(lookback));
        await SendRankingAsync(entries, title, kind, id => $"<#{id}>");
    }

    private async Task SendRankingAsync(List<TopEntry> entries, string title, StatKind kind,
        Func<ulong, string> mention)
    {
        var paginator = new LazyPaginatorBuilder()
            .AddUser(ctx.User)
            .WithPageFactory(PageFactory)
            .WithFooter(PaginatorFooter.PageNumber | PaginatorFooter.Users)
            .WithMaxPageIndex((entries.Count - 1) / 10)
            .WithDefaultEmotes()
            .WithActionOnCancellation(ActionOnStop.DeleteMessage)
            .Build();

        await interactivity.SendPaginatorAsync(paginator, (ctx.Interaction as SocketInteraction)!,
            TimeSpan.FromMinutes(60), InteractionResponseType.DeferredChannelMessageWithSource);
        return;

        async Task<PageBuilder> PageFactory(int page)
        {
            await Task.CompletedTask;
            return new PageBuilder().WithOkColor().WithTitle(title)
                .WithDescription(string.Join("\n", entries.Skip(page * 10).Take(10).Select((x, i) =>
                    $"`{page * 10 + i + 1}.` {mention(x.Id)}: **{ServerStatsEmbeds.FormatValue(kind, x.Value)}**")));
        }
    }

    /// <summary>
    ///     Draws a chart of messages, voice, members, status, joins, leaves or growth over a window.
    /// </summary>
    /// <param name="kind">What to chart.</param>
    /// <param name="days">The window in days, 1 to 90, or omitted for the server default.</param>
    [SlashCommand("chart", "Draws an activity chart")]
    [RequireContext(ContextType.Guild)]
    [CheckPermissions]
    [InteractionRatelimit(10)]
    public async Task Chart(StatChartKind kind = StatChartKind.Messages, [MinValue(1)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        var lookback = Math.Max(1, Service.ResolveLookback(ctx.Guild.Id, days));
        var (stream, embed) = await ServerStatsEmbeds.ChartAsync(Service, ctx.Guild, kind, lookback);
        await using (stream)
        {
            await FollowupWithFileAsync(stream, "graph.png", embed: embed);
        }
    }

    /// <summary>
    ///     Exports a member ranking as a CSV file.
    /// </summary>
    /// <param name="kind">Messages or voice.</param>
    /// <param name="days">The window in days, 0 for all time, or omitted for the server default.</param>
    [SlashCommand("export", "Exports a member ranking as CSV")]
    [RequireContext(ContextType.Guild)]
    [SlashUserPerm(GuildPermission.ManageGuild)]
    [InteractionRatelimit(300)]
    public async Task Export(StatKind kind = StatKind.Messages, [MinValue(0)] [MaxValue(90)] int? days = null)
    {
        await DeferAsync();
        var lookback = Service.ResolveLookback(ctx.Guild.Id, days);
        var csv = await Service.ExportCsvAsync(ctx.Guild, kind, lookback);
        await using var stream = new MemoryStream(Encoding.UTF8.GetBytes(csv));
        await FollowupWithFileAsync(stream, $"{kind}-{lookback}d.csv".ToLowerInvariant(),
            Strings.StatsExportDone(ctx.Guild.Id));
    }

    /// <summary>
    ///     Opts you out of activity stats everywhere, scrubbing what has been recorded, or opts you back in.
    /// </summary>
    [SlashCommand("privacy", "Opt out of (or back in to) activity stats everywhere")]
    [CheckPermissions]
    public async Task Privacy()
    {
        var guildId = ctx.Guild?.Id;
        if (statsSettings.IsOptedOut(ctx.User.Id))
        {
            await statsSettings.OptInAsync(ctx.User.Id);
            await EphemeralReplyConfirmAsync(Strings.StatsOptedIn(guildId));
            return;
        }

        if (!await PromptUserConfirmAsync(Strings.StatsOptOutConfirm(guildId), ctx.User.Id, true))
            return;

        await statsSettings.OptOutAsync(ctx.User.Id);
        await EphemeralReplyConfirmAsync(Strings.StatsOptedOut(guildId));
    }

    /// <summary>
    ///     Tracking settings.
    /// </summary>
    [Group("settings", "Configure activity tracking")]
    public class StatsSettingsCommands(ServerStatsSettingsService statsSettings)
        : MewdekoSlashSubmodule<ServerStatsService>
    {
        /// <summary>
        ///     Shows the stats tracking settings.
        /// </summary>
        [SlashCommand("show", "Shows the tracking settings")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task Show()
        {
            var settings = await statsSettings.GetSettingsAsync(ctx.Guild.Id);
            await RespondAsync(embed: ServerStatsEmbeds.Settings(Strings, ctx.Guild.Id, settings,
                statsSettings.GetActivityFilters(ctx.Guild.Id)).Build());
        }

        /// <summary>
        ///     Enables or disables game and app activity tracking.
        /// </summary>
        /// <param name="enabled">The new state.</param>
        [SlashCommand("track-activities", "Whether games and apps members use are tracked")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task TrackActivities(bool enabled)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.TrackActivities = enabled);
            await ReplyConfirmAsync(enabled
                ? Strings.StatsActivitiesOn(ctx.Guild.Id)
                : Strings.StatsActivitiesOffSet(ctx.Guild.Id));
        }

        /// <summary>
        ///     Sets whether only activities backed by a Discord application, Spotify or a stream are counted.
        /// </summary>
        /// <param name="enabled">The new state.</param>
        [SlashCommand("verify-activities", "Only count activities Discord itself attests to (blocks spoofing)")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task VerifyActivities(bool enabled)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.VerifyActivities = enabled);
            await ReplyConfirmAsync(
                enabled ? Strings.StatsVerifyOn(ctx.Guild.Id) : Strings.StatsVerifyOff(ctx.Guild.Id));
        }

        /// <summary>
        ///     Sets whether the activity filter list is a whitelist or a blacklist.
        /// </summary>
        /// <param name="mode">Whitelist or Blacklist.</param>
        [SlashCommand("activity-filter-mode", "Whether the activity filter is a whitelist or blacklist")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task ActivityFilterModeCommand(ActivityFilterMode mode)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.ActivityFilterMode = (int)mode);
            await ReplyConfirmAsync(Strings.StatsActivityFilterModeSet(ctx.Guild.Id, mode.ToString()));
        }

        /// <summary>
        ///     Toggles a game or app name on the activity filter list.
        /// </summary>
        /// <param name="name">The activity name.</param>
        [SlashCommand("activity-filter", "Toggles a game or app on the activity filter list")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task ActivityFilter([MaxLength(128)] string name)
        {
            var added = await statsSettings.ToggleActivityFilterAsync(ctx.Guild.Id, name);
            await ReplyConfirmAsync(added
                ? Strings.StatsActivityFilterAdded(ctx.Guild.Id, name)
                : Strings.StatsActivityFilterRemoved(ctx.Guild.Id, name));
        }

        /// <summary>
        ///     Lists the activity filter.
        /// </summary>
        [SlashCommand("activity-filters", "Lists the activity filter")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        public async Task ActivityFilters()
        {
            var mode = (ActivityFilterMode)statsSettings.GetCachedSettings(ctx.Guild.Id).ActivityFilterMode;
            var names = statsSettings.GetActivityFilters(ctx.Guild.Id);
            var eb = new EmbedBuilder()
                .WithOkColor()
                .WithTitle(Strings.StatsActivityFiltersTitle(ctx.Guild.Id, mode.ToString()))
                .WithDescription(names.Count == 0 ? "-" : string.Join("\n", names.Take(50)));
            await RespondAsync(embed: eb.Build());
        }

        /// <summary>
        ///     Sets the default window for stats commands.
        /// </summary>
        /// <param name="days">The window in days, 1 to 90.</param>
        [SlashCommand("lookback", "Default window in days for stats commands")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task Lookback([MinValue(1)] [MaxValue(90)] int days)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.DefaultLookbackDays = days);
            await ReplyConfirmAsync(Strings.StatsLookbackSet(ctx.Guild.Id, days));
        }

        /// <summary>
        ///     Sets how many seconds must pass between two counted messages from the same member.
        /// </summary>
        /// <param name="seconds">The cooldown, 0 to 300.</param>
        [SlashCommand("cooldown", "Seconds between counted messages from one member")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task Cooldown([MinValue(0)] [MaxValue(300)] int seconds)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.MessageCooldownSeconds = seconds);
            await ReplyConfirmAsync(Strings.StatsCooldownSet(ctx.Guild.Id, seconds));
        }

        /// <summary>
        ///     Enables or disables voice time tracking.
        /// </summary>
        /// <param name="enabled">The new state.</param>
        [SlashCommand("track-voice", "Whether voice time is tracked")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task TrackVoice(bool enabled)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.TrackVoice = enabled);
            await ReplyConfirmAsync(enabled ? Strings.StatsVoiceOn(ctx.Guild.Id) : Strings.StatsVoiceOff(ctx.Guild.Id));
        }

        /// <summary>
        ///     Enables or disables hourly member and status snapshots.
        /// </summary>
        /// <param name="enabled">The new state.</param>
        [SlashCommand("track-snapshots", "Whether hourly member and status snapshots are recorded")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task TrackSnapshots(bool enabled)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.TrackSnapshots = enabled);
            await ReplyConfirmAsync(enabled
                ? Strings.StatsSnapshotsOn(ctx.Guild.Id)
                : Strings.StatsSnapshotsOff(ctx.Guild.Id));
        }

        /// <summary>
        ///     Sets whether bots are counted.
        /// </summary>
        /// <param name="enabled">The new state.</param>
        [SlashCommand("count-bots", "Whether bots are counted in stats")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task CountBots(bool enabled)
        {
            await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.CountBots = enabled);
            await ReplyConfirmAsync(enabled ? Strings.StatsBotsOn(ctx.Guild.Id) : Strings.StatsBotsOff(ctx.Guild.Id));
        }

        /// <summary>
        ///     Toggles whether a voice state is left out of voice time.
        /// </summary>
        /// <param name="state">The state to toggle.</param>
        [SlashCommand("ignore-voice-state", "Toggle whether muted, deafened, AFK or alone time counts")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task IgnoreVoiceState(VoiceStateFlags state)
        {
            if (state == VoiceStateFlags.Normal)
            {
                await ReplyErrorAsync(Strings.StatsVoiceStateInvalid(ctx.Guild.Id));
                return;
            }

            var settings = await statsSettings.UpdateSettingsAsync(ctx.Guild.Id, s => s.VoiceStates ^= (int)state);
            var ignored = (settings.VoiceStates & (int)state) != 0;
            await ReplyConfirmAsync(ignored
                ? Strings.StatsVoiceStateIgnored(ctx.Guild.Id, state.ToString())
                : Strings.StatsVoiceStateCounted(ctx.Guild.Id, state.ToString()));
        }
    }

    /// <summary>
    ///     Channels, roles and members left out of stats.
    /// </summary>
    [Group("ignore", "Leave channels, roles or members out of stats")]
    public class StatsIgnoreCommands(ServerStatsSettingsService statsSettings)
        : MewdekoSlashSubmodule<ServerStatsService>
    {
        /// <summary>
        ///     Toggles whether a channel is left out of stats.
        /// </summary>
        /// <param name="channel">The channel.</param>
        [SlashCommand("channel", "Toggles whether a channel is left out of stats")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task Channel(IGuildChannel channel)
        {
            await ToggleAsync(channel.Id, StatsExclusionKind.Channel, $"<#{channel.Id}>");
        }

        /// <summary>
        ///     Toggles whether holders of a role are left out of stats.
        /// </summary>
        /// <param name="role">The role.</param>
        [SlashCommand("role", "Toggles whether holders of a role are left out of stats")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task Role(IRole role)
        {
            await ToggleAsync(role.Id, StatsExclusionKind.Role, role.Mention);
        }

        /// <summary>
        ///     Toggles whether a member is left out of stats in this server.
        /// </summary>
        /// <param name="user">The member.</param>
        [SlashCommand("user", "Toggles whether a member is left out of stats")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task User(IGuildUser user)
        {
            await ToggleAsync(user.Id, StatsExclusionKind.User, user.Mention);
        }

        /// <summary>
        ///     Lists ignored channels, roles and members.
        /// </summary>
        [SlashCommand("list", "Lists ignored channels, roles and members")]
        [RequireContext(ContextType.Guild)]
        [SlashUserPerm(GuildPermission.ManageGuild)]
        public async Task List()
        {
            var channels = statsSettings.GetExclusions(ctx.Guild.Id, StatsExclusionKind.Channel);
            var roles = statsSettings.GetExclusions(ctx.Guild.Id, StatsExclusionKind.Role);
            var users = statsSettings.GetExclusions(ctx.Guild.Id, StatsExclusionKind.User);

            var eb = new EmbedBuilder()
                .WithOkColor()
                .WithTitle(Strings.StatsIgnoredTitle(ctx.Guild.Id))
                .AddField(Strings.StatsIgnoredChannels(ctx.Guild.Id),
                    channels.Count == 0 ? "-" : string.Join(", ", channels.Select(x => $"<#{x}>")))
                .AddField(Strings.StatsIgnoredRoles(ctx.Guild.Id),
                    roles.Count == 0 ? "-" : string.Join(", ", roles.Select(x => $"<@&{x}>")))
                .AddField(Strings.StatsIgnoredUsers(ctx.Guild.Id),
                    users.Count == 0 ? "-" : string.Join(", ", users.Select(x => $"<@{x}>")));

            await RespondAsync(embed: eb.Build());
        }

        private async Task ToggleAsync(ulong targetId, StatsExclusionKind kind, string mention)
        {
            if (await statsSettings.AddExclusionAsync(ctx.Guild.Id, targetId, kind))
            {
                await ReplyConfirmAsync(Strings.StatsIgnored(ctx.Guild.Id, mention));
                return;
            }

            await statsSettings.RemoveExclusionAsync(ctx.Guild.Id, targetId, kind);
            await ReplyConfirmAsync(Strings.StatsUnignored(ctx.Guild.Id, mention));
        }
    }
}