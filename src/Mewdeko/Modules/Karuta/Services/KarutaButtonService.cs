...
public class KarutaButtonService : MewdekoSlashModuleBase<ShibaKarutaService>
{
    ...
    public KarutaButtonService(DbService db) => this.db = db;
    ...
    public async Task HandleVote(int voteNumber)
    {
        ...
        if (potentialEntry is null)
        {
            await ctx.Interaction.SendEphemeralFollowupErrorAsync(
                "Seems like that entry is no longer valid and can't be voted for.");
            return;
        }

        var potentialVote =
            uow.KarutaEventVotes.Where(x => x.MessageId == potentialEntry.MessageId && x.UserId == ctx.User.Id);
        if (potentialVote.Any())
        {
            if (potentialVote.Select(x => x.VotedNum).Contains(voteNumber))
            {
                if (await PromptUserConfirmAsync(
                        $"You already voted for `{buttonText}`, do you want to remove your vote?", ctx.User.Id, true))
                {
        ...
        else
        {
            var vote = new KarutaEventVotes
            {
                GuildId = ctx.Guild.Id, MessageId = interaction.Message.Id, VotedNum = voteNumber, UserId = ctx.User.Id
            };
            uow.KarutaEventVotes.Add(vote);
            await uow.SaveChangesAsync();
            await ctx.Interaction.SendEphemeralFollowupConfirmAsync($"Succesfully voted for `{buttonText}`");
