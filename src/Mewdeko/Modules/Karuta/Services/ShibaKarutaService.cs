...
{
    private readonly ConcurrentDictionary<ulong, int> messagesSent = new();
    private readonly ConcurrentDictionary<ulong, int> messagesSent1 = new();
    ...
    private async Task RepostKarutaImage(SocketMessage args)
    {
        ...
        using var sr = await httpClient.GetAsync(args.Attachments.First().Url, HttpCompletionOption.ResponseHeadersRead)
        ...
        var entries = uow.KarutaEventEntry.AsQueryable().Where(x => x.GuildId == channel.GuildId)
            .ToList() /*I fucking hate SQLite.*/.LastOrDefault();
        var entrynum = 1;
        if (entries is not null)
        {
            ...
            var msg = await channel.SendFileAsync(imgStream, filename: args.Attachments.First().Filename,
                embed: new EmbedBuilder().WithOkColor().WithTitle($"Entry #{entrynum}")
                    .WithImageUrl($"attachment://{args.Attachments.First().Filename}").Build(),
                components: componentBuilder.Build());
            var toadd = new KarutaEventEntry
            {
                ChannelId = eventChannel, GuildId = channel.GuildId, EntryNumber = entrynum, MessageId = msg.Id
            };
...

    {
        ...
        if (!messagesSent1.TryGetValue(gUser.Id, out var amount) || amount < 3)

            messagesSent1.AddOrUpdate(gUser.Id, amount++, (_, _) => amount++);
...

        {
            ...
            messagesSent1.TryRemove(gUser.Id, out _);
        }
    }
    ...
    public async Task SetEventChannel(ulong guildId, ulong channelId)
    {
        ...
        await guildSettingsService.UpdateGuildConfig(guildId, gc);
    }

    public async Task<ulong> GetEventChannel(ulong guildId)
        => (await guildSettingsService.GetGuildConfig(guildId)).KarutaEventChannel;
    ...
    public async Task SetButtonText(ulong guildId, int buttonNumber, string text)
    {
        ...
        var toadd = get ?? new KarutaButtonOptions
        {
            GuildId = guildId
        };
    ...
    public async Task<Embed?> GetLeaderboardEmbed(ulong guildId, int voteNum)
    {
        ...
        {
            case 1:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button1Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button1Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            case 2:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button2Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button2Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            case 3:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button3Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button3Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            case 4:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button4Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button4Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            case 5:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button5Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button5Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            case 6:
                entries = uow.KarutaEventEntry.ToList().Where(x => x.GuildId == guildId)
                    .OrderByDescending(x => x.Button6Count).Take(8);
                return new EmbedBuilder().WithOkColor().WithTitle($"Top 8 for {buttonText}").WithDescription(
                    entries.Any()
                        ? string.Join("\n",
                            entries.Select(x =>
                                $"**[Entry {x.EntryNumber} - {x.Button6Count}]({GetJumpUrl(x.GuildId, x.ChannelId, x.MessageId)})**"))
                        : "**No recorded entries yet.**").Build();
            default:
                return null;
        }
