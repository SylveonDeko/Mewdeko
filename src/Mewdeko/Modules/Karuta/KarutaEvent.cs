...
public class KarutaEvent : MewdekoModuleBase<ShibaKarutaService>
{
        ...
        if (channel is null)
        {
            ...
            if (eventChannelId is 0)
            {
                await ctx.Channel.SendErrorAsync(
                    "No Event Channel set! Set one using this command with a channel mentioned.");
            }
            else
            {
    ...
    public async Task KarutaButtonText(int buttonNumber, [Remainder] string text = null)
    {
        ...
        if (text is null)
        {
            ...
            {
                await ctx.Channel.SendErrorAsync(
                    $"Button {buttonNumber} does not have any text set. Use this command with text to set it.");
            }
            else
            {
    ...
    public async Task RemoveEventEntry(int eventNum)
    {
        if (!await PromptUserConfirmAsync(
                "Are you absolutely sure you want to remove this entry? This deletes it and the votes for it.",
                ctx.User.Id))
            return;
        var success = await Service.RemoveEntry(ctx.Guild.Id, eventNum);
        if (success)
            await ctx.Channel.SendConfirmAsync(
                "Entry along with it's votes have been removed, and the message has been deleted.");
        else
            await ctx.Channel.SendErrorAsync(
                "Looks like that entry doesn't exist. Please make sure you have the correct number.");
    }
    ...
    {
        if (await PromptUserConfirmAsync(
                "Are you absolutely sure you want to clear all entries and votes?? This cannot be undone!",
                ctx.User.Id))
        {
    ...
    public async Task KarutaEventLeaderboard(int questionNum = 0)
    {
        ...
        else
        {
            ...
            if (embed is null)
                await ctx.Channel.SendErrorAsync(
                    "Seems like that question hasn't been set. Please set it or try a different question number.");
            else
