...
//
// public class NewLogCommandService
// {
...
//
//
//     public NewLogCommandService(DbService db, IDataCache cache, DiscordSocketClient client, EventHandler handler, MuteService muteService)
//     {
//         this.db = db;
//         this.cache = cache;
//         this.client = client;
...
//                     .WithDescription($"`Updated By:` {auditLogs.FirstOrDefault().User.Mention} | {auditLogs.FirstOrDefault().User.Id}")
...
//                     .WithDescription($"`New System Channel:` {arsg2.SystemChannel.Mention} | {arsg2.SystemChannel.Id}\n" +
...
//                     .WithDescription($"`New Public Updates Channel:` {arsg2.PublicUpdatesChannel.Mention} | {arsg2.PublicUpdatesChannel.Id}\n" +
...
//                 return;
//
//             var auditLogs = await args.Value.Guild.GetAuditLogsAsync(1, actionType: ActionType.ThreadDelete).FlattenAsync();
//
//             var entry = auditLogs.FirstOrDefault();
//
//             var eb = new EmbedBuilder()
...
//     }
//
//     private async Task OnMessageUpdated(Cacheable<IMessage, ulong> cacheable, SocketMessage args2, ISocketMessageChannel args3)
//     {
//         if (!cacheable.HasValue) return;
...
//
//             var component = new ComponentBuilder().WithButton("Jump to Message", style: ButtonStyle.Link, url: oldMessage.GetJumpUrl()).Build();
//
...
//
//             var component = new ComponentBuilder().WithButton("View User", style: ButtonStyle.Link, url: $"discord://-/users/{guildUser.Id}").Build();
//
...
//
//             var component = new ComponentBuilder().WithButton("View User (May not work)", style: ButtonStyle.Link, url: $"discord://-/users/{arsg2.Id}").Build();
//
//             await channel.SendMessageAsync(components: component, embed: eb.Build());
//         }
//     }
...
//
//             var component = new ComponentBuilder().WithButton("View User (May not work)", style: ButtonStyle.Link, url: $"discord://-/users/{args.Id}").Build();
//
//             await channel.SendMessageAsync(components: component, embed: eb.Build());
//         }
//     }
...
//                 }
//
//             var auditLogs = await channel.Guild.GetAuditLogsAsync(1, actionType: ActionType.ChannelCreated).FlattenAsync();
//
//             var entry = auditLogs.FirstOrDefault();
//
//             var eb = new EmbedBuilder()
...
//                 }
//
//             var auditLogs = await channel.Guild.GetAuditLogsAsync(1, actionType: ActionType.ChannelDeleted).FlattenAsync();
//
//             var entry = auditLogs.FirstOrDefault();
//
//             var eb = new EmbedBuilder()
