...
/// <summary>
///     Economy tuning and reporting slash commands.
/// </summary>
public partial class SlashCurrency
{
    /// <summary>
    ///     Administrative economy commands: configuration, analytics, the daily reward and balance edits.
    /// </summary>
    [Group("admin", "Economy configuration and reporting")]
    public class CurrencyAdmin(CurrencyAnalyticsService analyticsService) : SlashCurrencyBase
    {
        /// <summary>
        ///     Maps a setting name to the code that parses a value for it and applies it.
        /// </summary>
        private static readonly Dictionary<string, Func<CurrencyAdmin, string, Task<string>>> Setters = new()
        {
            ...
            ["paycooldown"] = (m, v) => m.ApplyAsync(v, x => ParseInt(x, 0, 86400),
                (c, x) => c.PayCooldownSeconds = x),
            ...
            ["workcooldown"] = (m, v) => m.ApplyAsync(v, x => ParseInt(x, 0, 86400),
                (c, x) => c.WorkCooldownSeconds = x),
            ...
            ["robcooldown"] = (m, v) => m.ApplyAsync(v, x => ParseInt(x, 0, 86400),
                (c, x) => c.RobCooldownSeconds = x),
        ...
        /// </summary>
        [SlashCommand("config", "Shows every economy setting and its current value")]
        ...
        public async Task EconomyConfig()
        {
            ...
            var betting = new StringBuilder()
                ...
                .AppendLine(
                    $"`losslimit` {(config.LossLimitPerDay > 0 ? $"{config.LossLimitPerDay:N0} {emote}" : Off())}");
            ...
            var earning = new StringBuilder()
                ...
                .AppendLine(
                    $"`crime` {Fmt(config.CrimeEnabled)} {config.CrimeMinReward:N0}-{config.CrimeMaxReward:N0}")
        ...
        [SlashCommand("set", "Changes one economy setting")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        [SlashUserPerm(GuildPermission.Administrator)]
        public async Task EconomySet(
            [Summary("setting", "The setting to change")] [Autocomplete(typeof(EconomySettingAutocompleter))]
            string setting,
            [Summary("value", "The new value")] string value)
        {
        ...
        /// </summary>
        [SlashCommand("reset", "Restores every economy setting to its default")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        [SlashUserPerm(GuildPermission.Administrator)]
        ...
        /// </summary>
        [SlashCommand("stats", "Reports the size and concentration of the server's money supply")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        ...
        [SlashCommand("flow", "Shows where currency has been entering and leaving circulation")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        public async Task EconomyFlow([Summary("days", "How many days back to report, 1 to 365")] int days = 7)
        {
            await DeferAsync();
        ...
        /// </summary>
        /// <param name="days">How many days back to report. Defaults to 7.</param>
        [SlashCommand("game-stats", "Shows what each game paid back against what it took in")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        public async Task GameStats([Summary("days", "How many days back to report, 1 to 365")] int days = 7)
        {
            await DeferAsync();
            days = Math.Clamp(days, 1, 365);
            ...
            foreach (var game in performance.Take(20))
            {
                eb.AddField(game.Game,
                    Strings.GameStatsEntry(ctx.Guild.Id, game.ActualRtp, game.Wagered, emote, game.Plays,
                        game.Players, game.HouseTake), true);
            }
        ...
        [SlashCommand("set-daily", "Sets the daily reward amount and cooldown")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        [SlashUserPerm(GuildPermission.Administrator)]
        public async Task SetDaily([Summary("amount", "The amount of the daily reward")] int amount,
            [Summary("cooldown", "How long between claims, for example 1d")]
            TimeSpan time)
        {
            await Service.SetReward(amount, (int)time.TotalSeconds, ctx.Guild.Id);
            await ConfirmAsync(Strings.SetdailySuccess(ctx.Guild.Id, amount,
                await Service.GetCurrencyEmote(ctx.Guild.Id), time.ToReadableDuration()));
        }

        /// <summary>
        ///     Adds money to a user's balance. The amount can be negative.
        /// </summary>
        ...
        [SlashCommand("modify-balance", "Adds or removes currency from a user's balance")]
        [RequireContext(ContextType.Guild)]
        [CheckPermissions]
        [SlashUserPerm(GuildPermission.Administrator)]
        public async Task ModifyBalance([Summary("user", "The user to adjust")] IUser user,
            [Summary("amount", "The amount to add, negative to remove")]
            long amount,
            [Summary("reason", "The reason for the adjustment")]
            string? reason = null)
        {
        ...
        /// <param name="parse">Parses the raw text.</param>
        /// <param name="assign">Applies the parsed value to the configuration.</param>
        /// <returns>The stored value formatted for display.</returns>
