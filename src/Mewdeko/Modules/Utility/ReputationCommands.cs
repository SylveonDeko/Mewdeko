        ...
        public class Reputation : MewdekoSubmodule<ReputationService>
        {
            ...
            public async Task Reviews(IUser user = null, string type = null)
            {
                ...
                if (type is null)
                {
                    ...
                    eb.AddField("Positive Reviews", _service.Reputations(use.Id).Count(x => x.ReviewType == 1));
                    eb.AddField("Negative Reviews", _service.Reputations(use.Id).Count(x => x.ReviewType == 0));
