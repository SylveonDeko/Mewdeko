...
public partial class UserArrayConverter : TypeConverter<IUser[]>
{
    ...
    public override Task<TypeConverterResult> ReadAsync(IInteractionContext context, IApplicationCommandInteractionDataOption input, IServiceProvider services)
    {
        ...
        foreach (var userString in userStrings)
        {
            ...
            if (userRegex.Match(userString) is { Success: true } match && ulong.TryParse(match.Groups[1].Value, out var userId) || ulong.TryParse(userString, out userId))
                user = guild.GetUser(userId);
            // Match user name
            else
                user = guild.Users.FirstOrDefault(u => string.Equals(u.Username, userString, StringComparison.OrdinalIgnoreCase));

            if (user != null)
                users.Add(user);
            else
                return Task.FromResult(TypeConverterResult.FromError(InteractionCommandError.ParseFailed, $"User {userString} not found."));
        }
