...
public class RedisDictionary<TKey, TValue> : IDictionary<TKey, TValue>
{
    ...
    public void Add(TKey? key, TValue? value) => GetRedisDb().HashSet(redisKey, Serialize(key), Serialize(value));

    public bool ContainsKey(TKey? key) => GetRedisDb().HashExists(redisKey, Serialize(key));

    public bool Remove(TKey? key) => GetRedisDb().HashDelete(redisKey, Serialize(key));
