...
{
    public static async void EnsureUserCreated(this MewdekoContext ctx, ulong userId, string username, string discrim, string avatarId) =>
        await ctx.DiscordUser
                 .ToLinqToDBTable()
                 .InsertOrUpdateAsync(() => new DiscordUser
                     {
                         UserId = userId,
                         Username = username,
    ...
    public static async Task<DiscordUser> GetOrCreateUser(this MewdekoContext ctx, ulong userId, string username, string discrim, string avatarId)
    {
        ctx.EnsureUserCreated(userId, username, discrim, avatarId);
        return await ctx.DiscordUser
                        .Include(x => x.Club)
                        .FirstOrDefaultAsyncEF(u => u.UserId == userId).ConfigureAwait(false);
    }
    ...
    public static async Task<int> GetUserGlobalRank(this DbSet<DiscordUser> users, ulong id) =>
        await users
              .AsQueryable()
              .CountAsyncLinqToDB(x => x.TotalXp > users
                                                   .AsQueryable()
                                                   .Where(y => y.UserId == id)
                                                   .Select(y => y.TotalXp)
                                                   .FirstOrDefault()).ConfigureAwait(false) + 1;
