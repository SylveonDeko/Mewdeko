...
namespace Ayu.Discord.Voice
{
    internal static unsafe class Sodium
    {
        ...
        public static int Encrypt(byte[] input, int inputOffset, long inputLength, byte[] output, int outputOffset,
            in ReadOnlySpan<byte> nonce, byte[] secret)
        {
            ...
            {
                return SecretBoxEasy(outPtr + outputOffset, inPtr + inputOffset, inputLength - inputOffset, noncePtr,
                    secretPtr);
            }
        }

        public static int Decrypt(byte[] input, ulong inputLength, byte[] output, in ReadOnlySpan<byte> nonce,
            byte[] secret)
        {
            fixed (byte* outPtr = output)
            fixed (byte* inPtr = input)
