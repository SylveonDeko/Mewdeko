...
AppVersion={#version}
AppPublisher=Kwoth
DefaultDirName={commonpf}\Mewdeko
DefaultGroupName=Mewdeko
UninstallDisplayIcon={app}\{#sysfolder}\Mewdeko_icon.ico
Compression=lzma2
SolidCompression=yes
OutputDir=userdocs:_projekti/MewdekoInstallerOutput/{#version}/
...
Source: "src\Mewdeko\bin\Release\{#platform}\{#target}\publish\data\hangman.json"; DestDir: "{app}\{#sysfolder}\data"; DestName: "hangman.json"; Permissions: users-full; Flags: skipifsourcedoesntexist ignoreversion createallsubdirs recursesubdirs;
;rename credentials example to credentials, but don't overwrite if it exists
;Source: "src\Mewdeko\bin\Release\{#platform}\{#target}\publish\credentials_example.json"; DestName: "credentials.json"; DestDir: "{app}\{#sysfolder}"; Permissions: users-full; Flags: skipifsourcedoesntexist onlyifdoesntexist;
...
Source: "src\Mewdeko\bin\Release\{#platform}\{#target}\publish\data\pokemon"; DestDir: "{app}\{#sysfolder}\data\pokemon"; Permissions: users-full; Flags: skipifsourcedoesntexist ignoreversion createallsubdirs recursesubdirs;
;readme   
...
; desktop shortcut 
Name: "{commondesktop}\Mewdeko"; Filename: "{app}\Mewdeko"; Tasks: desktopicon
...
Root: "HKLM"; Subkey: "SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers"; \
    ValueType: String; ValueName: "{app}\{#sysfolder}\Mewdeko.exe"; ValueData: "RUNASADMIN"; \
...
Root: "HKCU"; Subkey: "SOFTWARE\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers"; \
    ValueType: String; ValueName: "{app}\{#sysfolder}\Mewdeko.exe"; ValueData: "RUNASADMIN"; \
    Flags: uninsdeletekeyifempty uninsdeletevalue;
Root: "HKLM"; Subkey: "SOFTWARE\Mewdeko"; \
...
[Messages]
WelcomeLabel2=Hello, if you have any issues, join https://discord.Mewdeko.bot and ask for help in #help channel.%n%nIt is recommended that you CLOSE any ANTI VIRUS before continuing.
...
begin
  ...
  begin
    ...
    if FileExists(X + '\{#sysfolder}\data\Mewdeko.db') then begin
      ...
      MsgBox(X + '\{#sysfolder}\data\Mewdeko.db doesn''t exist', mbConfirmation, MB_YESNO)
    end
  end;
end;
