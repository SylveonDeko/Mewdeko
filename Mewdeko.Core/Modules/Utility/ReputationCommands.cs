...
namespace Mewdeko.Modules.Utility
{
...

    {
        public class Reputation : MewdekoSubmodule<ReputationService>
        {
            ...
            {
                if (!await CheckContext(ctx.User, user, ctx.Guild))
                {
                    return;
                }
                if (review is null) 
                {
                    ...
                    await ctx.Channel.SendConfirmAsync($"Succesfully given {user.Mention} a positive review. Leave one with a comment next time!");
            ...
            public async Task repnegative(IUser user, [Remainder]string review = null)
            {
                ...
                if (review is null)
                {
                    ...
                    await ctx.Channel.SendConfirmAsync($"Succesfully given {user.Mention} a negative review. Leave one with a comment next time!");
            ...
            public async Task Reviews (IUser user = null, string type = null)
            {
                    ...
                    else
                    {
                        ...
                        await ctx.SendPaginatedConfirmAsync(0, cur =>
                        {
                            return new EmbedBuilder().WithOkColor()
                                .WithAuthor(x => x.WithIconUrl(revs.ToArray().Skip(cur).Take(1).FirstOrDefault().ReviewerAv).WithName(revs.ToArray().Skip(cur).Take(1).FirstOrDefault().ReviewerUsername))
                ...
                if (type is not null && type.ToLower() == "negative")
                {
                    ...
                    if (!reps2.Any())
                    {
                        ...
                        if (use != ctx.User)
                        {
                            await ctx.Channel.SendErrorAsync("This user has no negative reviews!");
                            return;
                        }
                    }
                    else
            ...
            private async Task<bool> CheckContext(IUser user,IUser user2, IGuild guild)
            {
                ...
                {
                    await ctx.Channel.SendErrorAsync("You have been blacklisted from reviews. Please visit discord.gg/oflaeti and open a ticket using .ticket for more info.");
                    return false;
                }
                var responses = _service.ServerReputations(guild.Id);
                var revreps = _service.ReviewerReps(user.Id);
                int c = responses.OrderByDescending(x => x.DateAdded).Take(30).Count(x => DateTimeOffset.Now.Subtract(x.DateAdded.Value) <= TimeSpan.FromDays(1));
                int r = revreps.OrderByDescending(x => x.DateAdded).Take(2).Count(x => DateTimeOffset.Now.Subtract(x.DateAdded.Value) <= TimeSpan.FromSeconds(30));
                if (user == user2)
                {
                ...
                {
                        await ctx.Channel.SendErrorAsync("This server has reached the daily 30 rep limit! There is going to be a whitelist soon! Come to discord.gg/oflaeti for news on this!");
                        return false;
                }
                else return true;
            }
            private async Task PostWebhook(IUser user, IGuild guild, string type, string review, IUser reviewer)
            {
                ...
                var webhook = new DiscordWebhookClient("https://discord.com/api/webhooks/856361973443854336/MzmEjAxO9ec3XIt3Q9RWQeoc7aOnlEPJQx2mIIvvOzz-19WiEDsY6ly7U8rmDzQ34Mfa");
                var eb = new EmbedBuilder();
                eb.Color = Mewdeko.OkColor;
                eb.Description = s;
                eb.Author = new EmbedAuthorBuilder() { IconUrl = user.RealAvatarUrl().ToString(), Name = $"{user.Username}|{user.Id}" };
                eb.Footer = new EmbedFooterBuilder() { IconUrl = reviewer.RealAvatarUrl().ToString(), Text = $"Reviewed by: {reviewer.Username}|{reviewer.Id}" };
