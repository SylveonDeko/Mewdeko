...
namespace Mewdeko.Modules.Searches
{
    ...
    {
        [Group]
        public class PathOfExileCommands : MewdekoSubmodule<SearchesService>
        {
                    ...
                    using (var http = _httpFactory.CreateClient())
                    {
                        ...
                        else
                        {
                            var currencyInput = obj["lines"]
                                .Values<JObject>()
                                .FirstOrDefault(i => i["currencyTypeName"].Value<string>() == cleanCurrency);
                        ...
                        else
                        {
                            var currencyOutput = obj["lines"]
                                .Values<JObject>()
                                .FirstOrDefault(i => i["currencyTypeName"].Value<string>() == cleanConvert);
            ...
            private readonly Dictionary<string, string> _currencyDictionary = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                {"Chaos Orb", "Chaos Orb" },
            ...
            {
                if (_currencyDictionary.ContainsValue(str))
                {
                    return str;
                }

                var currency = _currencyDictionary[str];
