    ...
    public partial class Administration
    {
        ...
        public class UserPunishCommands : MewdekoSubmodule<UserPunishService>
        {
            ...
            public async Task SetWarnChannel([Remainder]ITextChannel channel)
            {
                ...
                if (base.WarnlogChannel == channel.Id)
                    {
                        await ctx.Channel.SendErrorAsync("This is already your warnlog channel!");
                    return;
                    }
                if(base.WarnlogChannel == 0)
                    {
                        await CmdHandler.SetWarnlogChannelId(ctx.Guild, channel);
                        var WarnChannel = await ctx.Guild.GetTextChannelAsync(base.WarnlogChannel);
                        await ctx.Channel.SendConfirmAsync("Your warnlog channel has been set to " + WarnChannel.Mention);
                        return;
                    }
                var oldWarnChannel = await ctx.Guild.GetTextChannelAsync(base.WarnlogChannel);
                await CmdHandler.SetWarnlogChannelId(ctx.Guild, channel);
                var newWarnChannel = await ctx.Guild.GetTextChannelAsync(base.WarnlogChannel);
                await ctx.Channel.SendConfirmAsync("Your warnlog channel has been changed from " + oldWarnChannel.Mention + " to " + newWarnChannel.Mention);
            }
            ...
            public async Task Warn(IGuildUser user, [Remainder] string reason = null)
            {
                ...
                if(base.WarnlogChannel != 0)
                {
                    ...
                    var channel = await ctx.Guild.GetTextChannelAsync(base.WarnlogChannel);
                    await channel.EmbedAsync(new EmbedBuilder().WithErrorColor()
                                        .WithThumbnailUrl(user.RealAvatarUrl().ToString())
                                        .WithTitle("Warned by: " + ctx.User.ToString())
            ...
            public Task Warnlog(IGuildUser user = null)
            {
                ...
                return ctx.User.Id == user.Id || ((IGuildUser)ctx.User).GuildPermissions.MuteMembers ? Warnlog(user.Id) : Task.CompletedTask;
            }
            ...
            public async Task Ban(StoopidTime time, IGuildUser user, [Remainder] string msg = null)
            {
                ...
                await ctx.Channel.EmbedAsync(new EmbedBuilder().WithOkColor()
                        ...
                        .WithImageUrl("https://cdn.discordapp.com/attachments/732021633719730236/769426176099614720/IMG_20201024_010137.jpg")
            ...
            public async Task Yeet(IGuildUser user, [Remainder] string msg = null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(msg))
                {
                    try
                    {
                        await user.SendErrorAsync(GetText("yeetdm", Format.Bold(ctx.Guild.Name), msg)).ConfigureAwait(false);
                    }
                    catch { }
                }
                ...
                await ctx.Channel.EmbedAsync(new EmbedBuilder().WithOkColor()
                        ...
                        .WithImageUrl("https://cdn.discordapp.com/attachments/728758254796275782/744406804200685618/tenor_6.gif")
                        .AddField(efb => efb.WithName(GetText("username")).WithValue(user.ToString()).WithIsInline(true))
                        .AddField(efb => efb.WithName("ID").WithValue(user.Id.ToString()).WithIsInline(true)))
                    .ConfigureAwait(false);
            }
            ...
            public async Task PermYeet(IGuildUser user, [Remainder] string msg = null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(msg))
                {
                    try
                    {
                        await user.SendErrorAsync("You have been permanently yeeted from ", Format.Bold(ctx.Guild.Name), msg).ConfigureAwait(false);
                    }
                    catch
                    {
                        // ignored
                ...
                await ctx.Channel.EmbedAsync(new EmbedBuilder().WithOkColor()
                        ...
                        .WithImageUrl("https://media.discordapp.net/attachments/707730610650873916/758453543836450816/ezgif.com-gif-maker.gif")
                        .AddField(efb => efb.WithName(GetText("username")).WithValue(user.ToString()).WithIsInline(true))
                        .AddField(efb => efb.WithName("ID").WithValue(user.Id.ToString()).WithIsInline(true)))
                    .ConfigureAwait(false);
            }
