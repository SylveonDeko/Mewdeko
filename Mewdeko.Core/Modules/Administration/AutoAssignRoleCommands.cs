        ...
        public class AutoAssignRoleCommands : MewdekoSubmodule<AutoAssignRoleService>
        {
            ...
            public async Task AutoAssignRole(params IRole[] role)
            {
                ...
                if (role.Any())
                {
                    ...
                    _service.EnableAar(ctx.Guild.Id, String.Join<ulong>(" ", roleIDs));
                    await ctx.Channel.SendConfirmAsync("Auto role has been set to the roles: " + String.Join<String>("|", role.Select(x => x.Mention))).ConfigureAwait(false);
                }
            }
