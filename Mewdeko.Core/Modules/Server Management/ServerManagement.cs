...
namespace Mewdeko.Modules.ServerManagement
{
    public partial class ServerManagement : MewdekoTopLevelModule<ServerManagementService>
    {
        ...
        public async Task AddEmote(string name, string url = null)
        {
            ...
            using (var sr = await http.GetAsync(uri, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false))
            {
                ...
                using (var imgStream = imgData.ToStream())
                {
                    ...
                    {
                        await ctx.Channel.SendErrorAsync("The emote could not be added because it is either: Too Big(Over 256kb), != a direct link, Or exceeds server emoji limit.");
                    }
                }
            }
        }
        ...
        {
            var tags = ctx.Message.Tags.Where(t => t.Type == TagType.Emoji).Select(x => (Emote)x.Value).FirstOrDefault();
            try
            {
        ...
        public async Task RenameEmote(string emote, string name)
        {
            ...
            try
            {
                ...
                await ctx.Guild.ModifyEmoteAsync(emote1, x =>
                {
                    x.Name = name;
                });
        ...
        public async Task StealEmotes([Remainder]string e)
        {
            ...
            {
                using var http = _httpFactory.CreateClient();
                using (var sr = await http.GetAsync(i.Url, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false))
                {
                    var imgData = await sr.Content.ReadAsByteArrayAsync().ConfigureAwait(false);
                    using (var imgStream = imgData.ToStream())
                    {
