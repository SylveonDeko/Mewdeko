...
{
    public partial class SwitchUtils : MewdekoModule<SwitchUtilsService>
    {
        [MewdekoCommand]
        [Usage]
        [Description]
        ...
        public async Task serr(string err)
        {
            ...
            int module;
            int errcode;
            Regex switch_re = new Regex(@"2\d{3}\-\d{4}");
            ...
            if (yesn.Success || err.StartsWith("0x"))
            {
                ...
                {
                    module = Convert.ToInt32(err[0..4]) - 2000;
                ...
                embed.AddField(name: "Module", value: $"{err_module} ({module})", inline: true);
                embed.AddField(name: "Description", value: desc, inline: true);
                embed.WithOkColor();
                ...
                {
                    embed.WithFooter(text: "Console: Switch");
                }
            ...
            else if (_service.switch_game_err.Select(x => x.Key).Contains(err))
            {
                ...
                var game = _tup_1[0];
                string desc1 = _tup_1[1];
                embed = new EmbedBuilder { Title = err, Description = desc1 };
                embed.WithFooter(text: "Console: Switch");
                embed.AddField(name: "Game", value: game, inline: true);
                await ctx.Channel.SendMessageAsync(embed: embed.Build());
            }
            else
            {
                await ctx.Channel.SendErrorAsync("Unknown Format - This is either no error code or you made some mistake!");
            }
        }
        [MewdekoCommand]
        [Usage]
        ...
        public async Task err2hex(string err)
        {
            ...
            {
                var module = Convert.ToInt32(err[0..4]) - 2000;
