...
namespace Mewdeko.Modules.Music.Services
{
    public class MusicService : INService, IUnloadableService
    {
            ...
            SoundCloudApiService sc, IBotCredentials creds, Mewdeko bot)
        {
            ...
            _botlistTimer = new Timer(async (state) =>
            {
                try
                {
                    ...
                    var request = new LoginRequest(_server.BaseUri, "dc237c779f55479fae3d5418c4bb392e", LoginRequest.ResponseType.Code)
                    {
        ...
        private static async Task OnAuthorizationCodeReceived(object sender, AuthorizationCodeResponse response)
        {
            ...
            var tokenResponse = await new OAuthClient(config).RequestToken(
              new AuthorizationCodeTokenRequest(
                "dc237c779f55479fae3d5418c4bb392e", "db01b63b808040efbdd02098e0840d90", response.Code, new Uri("http://localhost:1234/callback")
              )
            );
        ...
        public async Task TryQueueRelatedSongAsync(SongInfo song, ITextChannel txtCh, IVoiceChannel vch)
        {
            ...
            var si = await ResolveSong(related[new MewdekoRandom().Next(related.Length)], _client.CurrentUser.ToString(), MusicType.YouTube).ConfigureAwait(false);
            if (si == null)
